...
<body>
    ...
    <!-- Navbar Start -->
    <div class="container-fluid mb-5" style="margin-bottom: 0 !important">
        <div class="row border-top px-xl-5" style=" height: 351px;">
    ...
    <!-- Cart Start -->
    <div class="container-fluid pt-5" style="padding-top: 0 !important">
                    ...
                    <tbody class="align-middle">
                        <tr>
                            <?php
                                ...
                                {
                                    $sql = "SELECT product_details, supermarket, price, quantity FROM product_physical WHERE id = $prod_id";
                            
                                    $product_physical = $con->query($sql)->fetch_assoc();
                                    
                                    $prod_id_details = $product_physical['product_details'];
                                    $supermarket_id = $product_physical['supermarket'];
                                    $price = $product_physical['price'];
                                    $quantity_max = $product_physical['quantity'];
                                    
                                    if ($quantity_max < $quantity) 
                                    {
                                        $quantity = $quantity_max;
                                        $_SESSION['cart'][$prod_id] = ($quantity > 0) ? $quantity : 0;
                                    } elseif ($quantity <= 0) {
                                    ...
                                    $sql = "SELECT name FROM product_details WHERE id = $prod_id_details";
                                    
                                    $product_name = $con->query($sql)->fetch_assoc()['name'];
                                    
                                    $sql = "SELECT name FROM supermarket WHERE id = $supermarket_id";
                                    $supermarket_name = $con->query($sql)->fetch_assoc()['name'];   
                            ?>

                            <td class="align-middle"><img src="images/products/product-<?php echo $prod_id ?>.jpg" alt="" style="width: 50px;"> <?php echo $product_name ?></td>
                            <td class="align-middle"><?php echo $supermarket_name ?></td>
                            <td class="align-middle"><?php echo $price?> €</td>
                            <td class="align-middle">
                                <div class="input-group quantity mx-auto" style="width: 100px;">
                                    
                                    <input type="text" class="form-control form-control-sm bg-secondary text-center" value="<?php echo $quantity ?>" style="color:red" id="product-quantity-<?php echo $prod_id ?>">

                                </div>
                            </td>
                            <td class="align-middle"><?php echo $total?> €</td>
                            <td class="align-middle"><button class="btn btn-sm btn-primary" data-product-id="<?php echo $prod_id ?>"><i class="fa fa-times"></i></button></td>
                        </tr>
                ...
                <div class="card border-secondary mb-5">
                    ...
                    <div class="card-footer border-secondary bg-transparent">
                        <div class="d-flex justify-content-between mt-2">
                            ...
                            <h5 class="font-weight-bold"><?php echo $total_all ?> €</h5>
                        </div>
                        <button class="btn btn-block btn-primary my-3 py-3"><a class="btn btn-primary" href="checkout.php">Proceed To Checkout</a></button>
                    </div>
                </div>
            </div>
        </div>
